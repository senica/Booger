...
jQuery("#core-toolbar-tv-tools-wrapper .media").live("click", function(){
	...
	jQuery(tv).bind('core-media-insert', function(event, obj){
		...
		if(obj.type == 'image/jpeg' || obj.type == 'image/png' || obj.type == 'image/gif'){
			...
			el.src = obj.url;
			range.insertNode(el);
		}else if(obj.type == 'audio/mp3' || obj.type == 'audio/wav'){
			var el=core_toolbar.focus.doc.createElement("audio");
			range.insertNode(el);
			jQuery(el).addClass("core-media-audio-wrapper");
			jQuery(el).attr("controls", "controls");
			jQuery(el).html('<source src="'+obj.url+'" type="'+obj.type+'" /><!-- You can add more sources here for incompatible browsers. -->Your browser does not support the audio element.  Consider upgrading to <a href="http://www.google.com/chrome/" target="_blank">Google Chrome</a>');	
		}else{
			var el=core_toolbar.focus.doc.createElement("div");
			range.insertNode(el);
			jQuery(el).addClass("core-media-download-wrapper");

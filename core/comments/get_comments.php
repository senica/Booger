...
error_reporting(E_ALL);
	...
	function cycle($parent, $switch){
		...
		foreach($results as $result){
			...
			$author = ($result->comment_author_url != "") ? '<a href="'.$result->comment_author_url.'" target="_blank">'.$result->comment_author.'</a>' : $result->comment_author;
			echo '
				<div class="comment-wrapper '.$even.'">
					...
					<div class="comment-text '.$even.'">'.$result->comment_content.'</div>

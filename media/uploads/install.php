...
<title>Booger Install Script</title>
<style type="text/css">
html, body{ margin:0; padding:0; background:#F9F9F9; width:100%; text-align:center; }
#wrapper{ display:inline-block; margin-top:80px; }
#message{ color:#666; font-weight:bold; text-align:left; padding:20px; font-size:14px; font-family:"Lucida Sans Unicode", "Lucida Grande", sans-serif;  -webkit-box-shadow: 2px 2px 10px #666666; -moz-box-shadow: 2px 2px 10px #666666; box-shadow: 2px 2px 10px #666666 ; display:inline-block; width:300px; border:0; border-radius:10px; -moz-border-radius:10px; -webkit-borde-radius:10px;
	...
	background: -o-linear-gradient(top, rgba(255,255,255,1) 1%,rgba(238,238,238,1) 100%); /* Opera11.10+ */
	background: -ms-linear-gradient(top, rgba(255,255,255,1) 1%,rgba(238,238,238,1) 100%); /* IE10+ */
	filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#ffffff', endColorstr='#eeeeee',GradientType=0 ); /* IE6-9 */
	background: linear-gradient(top, rgba(255,255,255,1) 1%,rgba(238,238,238,1) 100%); /* W3C */
}
#sub{ color:#aaa; font-weight:normal; }
#logo{ display:inline-block; background-image:url("http://www.boogercms.org/admin/images/booger_logo.png"); width:183px; height:86px; }
.button{
	border-radius:10px; -moz-border-radius:10px; -webkit-borde-radius:10px;
	padding:5px;
	color:#999;
...
.button:hover{ color:#000; background-color:#000; }
.download-message{ text-align:center; }
.red{ color:#000; }
.black{ color:#ccc; }
</style>
<script>
...
function animate(obj){
	...
	var y	= obj.y;
	var loop= 0;
	while(x < obj.width + 50){
		ctx.beginPath();
		ctx.moveTo(x,y+20);
		ctx.lineTo(x+10,y);
		ctx.lineTo(x+20,y);
		ctx.lineTo(x+10,y+20);
		ctx.fillStyle = (loop % 2 != 0) ? "#CCCCCC" : "#999999" ;
		ctx.fill();
		x = x + 10;
		loop = loop + 1;
	}
...
jQuery(document).ready( function(){
	jQuery(".download.button").click( function(){
		jQuery(".download-wrapper").html("<canvas id='canvas' width='300' height='20'></canvas>");	
		var canvas = jQuery("#canvas")[0];
		if (canvas.getContext){
			var ctx = canvas.getContext('2d');
			var x	= 0 - canvas.width - 20;
			var y	= 0;
			setInterval(function(){
				ctx.clearRect(0,0,canvas.width,canvas.height);
				animate({ctx:ctx, width:canvas.width, height:canvas.height, x:x, y:y});
				x = (x >= 0-20) ? 0-canvas.width-20 : x + 1;
			}, 10);
		}
		comet.process = function(response){
			...
			if(response.action == "done"){
				jQuery(".list .move").wrapInner("<strike class='red'><span class='black'></span></strike>");
				jQuery(".download-wrapper").html("");
			}
		}
		jQuery(".download-iframe").html('<iframe src="install.php?action=fetch" style="width:0px; height:0px; border:0px;"></iframe>');
	});
});
</script>
<div id="wrapper">
	...
	<div id="message">
		...
		<div id="sub">By clicking the download button, you understand the risks involved with transferring data. You also agree that any data not backed up could potentially be damaged.</div>
		<div style="text-align:center; margin-top:20px;" class="download-wrapper"><a href="#" class="download button">Download</a></div>
		<div class="download-iframe"></div>
